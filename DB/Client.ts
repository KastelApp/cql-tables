...
class Client extends EventEmitter {
	...
	private async handleTable(keyspace: string, table: Table<_T>) {
		...
		if (table.options.types) {
			// ? check all the types, see if they are missing a column in the type and edit it if so
			for (const [name, type] of Object.entries(table.options.types)) {
				const [typeData, typeError] = await safePromise(
					this.connection.execute(
						`SELECT * FROM system_schema.types WHERE keyspace_name = '${keyspace}' AND type_name = '${table.snakeifyString(name)}'`,
					),
				);

				if (typeError || !typeData) {
					throw new Error(`Failed to get type metadata: ${typeError?.message}`);
				}
				...
				const typeColumns: {
					...
					typeData.rows?.[0]?.field_names?.map((row: string, i: number) => ({
						name: row,
						type: typeData.rows?.[0]?.field_types[i],
					})) ?? [];

				const missingTypeColumns = Object.keys(type).filter(
					(column) => !typeColumns.some((col) => col.name === table.snakeifyString(column)),
				);

				for (const column of missingTypeColumns) {
					const answer = await rl.question(
						`[${table.options.tableName}] The column ${column} is not in the remote type ${name}, would you like to add it? [y/n] `,
					);

					if (possibleYes.includes(answer)) {
						const [, error] = await safePromise(
							this.connection.execute(
								`ALTER TYPE ${table.snakeifyString(name)} ADD ${table.snakeifyString(column)} ${table.extractTypeName(type[column]!)}`,
							),
						);

						if (error) {
							throw new Error(
								`Failed to add column ${table.snakeifyString(column)} to type ${table.snakeifyString(name)}: ${error.message}`,
							);
						}

						App.staticLogger.info(`Added column ${table.snakeifyString(column)} to type ${name}`);
					}
				}

				const extraColumns = typeColumns.filter(
					(column) =>
						!Object.keys(type)
							.map((col) => table.snakeifyString(col))
							.includes(column.name),
				);

				for (const column of extraColumns) {
					App.staticLogger.debug(
						`[${table.options.tableName}] The column ${column.name} is in the remote type ${name}, but not in the local type sadly you cannot remove columns from a type`,
					);
				}
			}
		}
