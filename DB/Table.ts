...
class Table<T> {
		...
		if (this.options.migrationScripts) {
			...
			if (doWeHaveAllFields && hasAllPrimary) {
				...
				if (Bun.deepEquals(migratedData, data)) {
					...
					App.staticLogger.debug(
						`[${this.tableName}] [R1] (${res?.wasApplied()}) No changes were made to the data, but the version was updated to ${version + 1}`,
						opts,
					);
				} else {
					const opts = {
					...
					App.staticLogger.debug(
						`[${this.tableName}] [R2] (${res?.wasApplied()}) The data has been updated to version ${version + 1}`,
					);
				}

				// ? if version + 1 is less then the latest version, continue migrating
				if (version + 1 < this.version) {
					return await this.migrateData(migratedData, version + 1, structuredClone(where));
				}
	...
	> {
		...
		if (this.versionName !== "") {
			...
			if (version === undefined || version === null) {
				finishedData = await this.migrateData(finishedData, 0, structuredClone(filter));
			}

			if (typeof version === "number" && version < this.version) {
				finishedData = await this.migrateData(finishedData, version, structuredClone(filter));
			}
		}
	...
	>(
		filter: Filter,
		update: Omit<Partial<NullifyStuff<ExtractTypesFromCreateTable<this["options"]>>>, keyof Filter>,
		options?: {
			/**
	...
	public async create<Data extends Partial<NullifyStuff<ExtractTypesFromCreateTable<this["options"]>>>>(
		data: Data,
		options?: {
			/**
			 * If you want to set a specific version for the tbale, this is somewhat dangerous but if for whatever reason you want to sure
	...
	>(
		...
		options?: {
			...
			orderBy?: {
				type: "ASC" | "DESC";
				field: keyof RawThis["options"]["columns"];
			};
			before?: {
				[key in keyof ExtractTypesFromCreateTable<RawThis["options"]>]?: ExtractTypesFromCreateTable<
					RawThis["options"]
				>[key];
			};
			after?: {
	...
	> {
		...
		if (options.before) {
			opts.query += `AND ${Object.keys(options.before)
				.map((key) => `${this.snakeifyString(key)} < ?`)
				.join(" AND ")}`;
			opts.params.push(...Object.values(options.before).map((v) => this.recursiveConvert(v, true)));
		}

		if (options.after) {
			opts.query += `AND ${Object.keys(options.after)
				.map((key) => `${this.snakeifyString(key)} > ?`)
				.join(" AND ")}`;
			opts.params.push(...Object.values(options.after).map((v) => this.recursiveConvert(v, true)));
		}

		if (options.orderBy) {
			opts.query += `ORDER BY ${this.snakeifyString(options.orderBy.field)} ${options.orderBy.type}`;
		}

		opts.query += `${options.allowFiltering ? " ALLOW FILTERING" : ""}${options.limit ? ` LIMIT ${options.limit}` : ""};`;
		...
		for (const row of data) {
			...
			if (this.versionName !== "") {
				...
				if (version === undefined || version === null) {
					finishedData.push(await this.migrateData(newObj, 0, structuredClone(filter)));

					continue;
				}

				if (typeof version === "number" && version < this.version) {
					finishedData.push(await this.migrateData(newObj, version, structuredClone(filter)));
	...
	public extractTypeName(type: AllTypes): string {
		return ExtractValueName(type);
	}
